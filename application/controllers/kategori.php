<?php 
	class Kategori extends CI_Controller{
		...
		function index()
		{
			...
			$data['record'] = $this->model_kategori->tampilkan_data_pagging($halaman,$config['per_page'])->result();
		...
		function post()
		{
			if(isset($_POST['submit']))
			{
				...
				$this->model_kategori->post($nama_kategori);
				redirect('kategori');
		...
		function edit()
		{
			if(isset($_POST['submit']))
			{
				...
				$nama_kategori = $this->input->post('nama_kategori');
				$this->model_kategori->edit($id,$nama_kategori);
				redirect('kategori');
			}
			else
			{
